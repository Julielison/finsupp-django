...
class TransactionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if is_add_to_bill:
            if not card:
                self.add_error('is_add_to_bill', 'Um cartão deve ser informado para adicionar na fatura.')
            elif card.type not in ['CREDIT', 'BOTH']:
                self.add_error('is_add_to_bill', 'Apenas transações com cartão de crédito podem ser adicionadas à fatura.')
